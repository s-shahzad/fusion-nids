...
```powershell
<workspace>\scripts\setup_virtualbox_lab.ps1 -InstallVirtualBox
```

Then build the realistic lab:

```powershell
.\build-realistic-lab.ps1 -AttachIso -UbuntuIsoPath <workspace>\lab\isos\ubuntu-24.04.4-live-server-amd64.iso
```
...
```powershell
.\run-offline-test.ps1 -LabelsPath <workspace>\lab\pcaps\labels.csv
```
...
```powershell
.\run-offline-test.ps1 -PcapPath <workspace>\lab\pcaps\serious_synthetic_20260310.pcap
```
...
Current note: the repaired concurrent path can now produce same-window evidence in the live lab. The strongest current mixed proofs are [live-overlap-profile-dns-http-ordered-20260311-154000](<workspace>\lab/results/live-overlap-profile-dns-http-ordered-20260311-154000), where `DNS Burst / DGA-like Activity` and `HTTP Login Brute Force Threshold` both fired in one run, and [live-multi-attack-dns-http-scan-20260311-145300](<workspace>\lab/results/live-multi-attack-dns-http-scan-20260311-145300), where `DNS Burst / DGA-like Activity`, `Suspicious Port Scan`, and `Hybrid Fusion Decision` fired in one run.

For thesis-grade overlap validation, there is also a dedicated profile at [live_vm_overlap_profile.yml](<workspace>\lab/config/live_vm_overlap_profile.yml). It widens the brute-force/login windows and disables the live unsupervised path to reduce runtime pressure during mixed runs. Use it with:
...
The strongest current overlap-profile proofs are [live-overlap-profile-dns-rdp-20260311-152700](<workspace>\lab/results/live-overlap-profile-dns-rdp-20260311-152700), where `DNS Burst / DGA-like Activity`, `RDP Brute Force Threshold`, `Suspicious Port Scan`, and `Hybrid Fusion Decision` fired in one run, and [live-overlap-profile-dns-http-ordered-20260311-154000](<workspace>\lab/results/live-overlap-profile-dns-http-ordered-20260311-154000), where `DNS Burst / DGA-like Activity` and `HTTP Login Brute Force Threshold` fired together after ordered launch tuning.

Current note: HTTP login brute-force validation now passes live in the VM lab when the sensor uses the tcpdump-backed capture path and the stable port `80` service. The resolved note is in [http_login_live_gap.md](<workspace>\lab/reports/http_login_live_gap.md).
...
.\run-artifact-static-scan.ps1 -IncomingPath <workspace>\lab\artifacts\incoming\<seed-folder> -Recursive
```
...
.\run-artifact-static-scan.ps1 -IncomingPath <workspace>\lab\artifacts\incoming\<phishing-folder> -Recursive
```
...
.\run-artifact-static-scan.ps1 -IncomingPath <workspace>\lab\artifacts\incoming\<pe-loader-folder> -Recursive
```
...
.\run-artifact-static-scan.ps1 -IncomingPath <workspace>\lab\artifacts\incoming\<credential-folder> -Recursive
```
...
.\run-artifact-static-scan.ps1 -IncomingPath <workspace>\lab\artifacts\incoming\<rat-folder> -Recursive
```
...
.\run-artifact-static-scan.ps1 -IncomingPath <workspace>\lab\artifacts\incoming\<ransomware-folder> -Recursive
```
...
That launcher calls [ubuntu_os_defense_validation.py](<workspace>\scripts/ubuntu_os_defense_validation.py), supports `cron-http`, `systemd-dns`, and `defense-tamper` cases, and packages the attack-side target artifacts and the defense-side sensor artifacts into a single evidence folder under `results\`.
...
The runner now uses the dedicated [os_defense_profile.yml](<workspace>\lab/config/os_defense_profile.yml) and writes a thesis-style `phd_case_report.docx` into each run folder.

The first completed OS-defense result is [ubuntu-os-cron-http-beacon-20260311-161200](<workspace>\lab/results/ubuntu-os-cron-http-beacon-20260311-161200). That run captured `5` flows and `1` `HTTP Suspicious Keyword` alert while preserving:
...
The validated `systemd + DNS beacon` result is [ubuntu-os-systemd-dns-beacon-20260311-162948](<workspace>\lab/results/ubuntu-os-systemd-dns-beacon-20260311-162948). That rerun captured `36` flows and `1` `DNS Burst / DGA-like Activity` alert after removing the extra UDP sink from the OS-defense runner.

The validated defense-tamper result is [ubuntu-os-defense-tamper-20260311-attack-defense](<workspace>\lab/results/ubuntu-os-defense-tamper-20260311-attack-defense). That run captured `41` flows and `1` `Linux Defense Tamper Command` alert while preserving both the attack-side host artifacts and the defense-side sensor artifacts in the same case folder.

Use the OS scope and recording rules in [os_defense_test_plan.md](<workspace>\lab/reports/os_defense_test_plan.md) before adding the next Ubuntu staged-exfiltration or lower-rate beacon cases.

Earlier `systemd` fix iterations remain part of the thesis record and should not be deleted:
- [ubuntu-os-systemd-dns-beacon-20260311-164900](<workspace>\lab/results/ubuntu-os-systemd-dns-beacon-20260311-164900)
- [ubuntu-os-systemd-dns-beacon-20260311-165700](<workspace>\lab/results/ubuntu-os-systemd-dns-beacon-20260311-165700)
- [ubuntu-os-systemd-dns-beacon-20260311-170500](<workspace>\lab/results/ubuntu-os-systemd-dns-beacon-20260311-170500)

## Notes

- VirtualBox is the preferred lab path for this machine.
- The authoritative summary after each build is [realistic_lab_summary.json](<workspace>\lab/realistic_lab_summary.json).
- The thesis-friendly attack history is tracked in [attack_test_ledger.md](<workspace>\lab/reports/attack_test_ledger.md).
- The OS-level validation scope is tracked in [os_defense_test_plan.md](<workspace>\lab/reports/os_defense_test_plan.md).
- The cited research-gap note is tracked in [nids_research_gaps.md](<workspace>\lab/reports/nids_research_gaps.md).
