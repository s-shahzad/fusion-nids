...
- [signature.py](<workspace>\src/nids/detect/signature.py) now supports `http_methods` and `http_uris` rule fields.
- [rules.yml](<workspace>\rules/rules.yml) now drives `HTTP Suspicious Keyword` from parsed HTTP semantics instead of a fixed destination-port list.
- Regression coverage is in [test_signature.py](<workspace>\tests/test_signature.py).
...
- [ubuntu_os_defense_validation.py](<workspace>\scripts/ubuntu_os_defense_validation.py) now packages target-host artifacts and sensor artifacts together.
...
1. Port-evasion hardening for suspicious HTTP detection
   Files:
   [signature.py](<workspace>\src/nids/detect/signature.py)
   [rules.yml](<workspace>\rules/rules.yml)
   [test_signature.py](<workspace>\tests/test_signature.py)

2. Ubuntu OS-defense automation and thesis reporting
   Files:
   [ubuntu_os_defense_validation.py](<workspace>\scripts/ubuntu_os_defense_validation.py)
   [run-ubuntu-os-defense-test.ps1](<workspace>\lab/run-ubuntu-os-defense-test.ps1)
   [os_defense_profile.yml](<workspace>\lab/config/os_defense_profile.yml)

3. Thesis-ready evidence outputs
   Successful cases:
   [ubuntu-os-cron-http-beacon-20260311-161200](<workspace>\lab/results/ubuntu-os-cron-http-beacon-20260311-161200)
   [ubuntu-os-systemd-dns-beacon-20260311-162948](<workspace>\lab/results/ubuntu-os-systemd-dns-beacon-20260311-162948)
